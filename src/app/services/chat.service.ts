...
export class ChatService {
  ...
  loadMessages = () => {
    ...
    (collection(this.firestore, 'messages'), //acc colectia messages
    orderBy('timestamp', 'desc'), //sort
    limit(12)); 
  ...
  saveMessagingDeviceToken = async () => {
    try {
      ...
      if(currentToken) {
        ...
        //Saving the Dev Token to Firestore
        const tokenRef = doc(this.firestore, 'fcmTokens', currentToken);
        await setDoc(tokenRef, { uid: this.auth.currentUser?.uid });

        // This will fire when a message is received while the app is 
        // in the foreground.
        ...
        //Need to req permission to show notif
